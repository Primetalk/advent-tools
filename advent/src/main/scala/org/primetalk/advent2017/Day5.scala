...
object Day5 extends Utils {
  ...
  lazy val jumpProgram: Seq[Int] = lines.map(_.toInt)

  def run(program: Seq[Int]): Int = {
    val mem = program.toArray
    val len = mem.length
    def go(pointer: Int, cnt: Int): Int = {
      if(pointer < 0 || pointer >= len )
        cnt
      else {
        val delta = mem(pointer)
        mem(pointer) += 1
        go(pointer + delta, cnt + 1)
      }
    }
  ...
  lazy val answer1: Int = run(jumpProgram)

  // Part 2

  def run2(program: Seq[Int]): Int = {
    val mem = program.toArray
    val len = mem.length
    def go(pointer: Int, cnt: Int): Int = {
      if(pointer < 0 || pointer >= len )
  ...
  lazy val answer2: Int = run2(jumpProgram)
