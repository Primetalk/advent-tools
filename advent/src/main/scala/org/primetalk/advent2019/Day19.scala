...
object Day19 extends IntCodeComputer9 {
  ...
  def visibleBeam(p: Position): Char = if(beam(p._1, p._2)) '#' else '.'

  lazy val answer1: Word = {
    val d = Display.of[Char]((0,0),(50,50))(visibleBeam)
...
//    beamR.sum
  ...
  val size = 100
  /** will return a couple of additional points */
  def getDiagonalBottomLeftTopRightWiderByL(x: Int, y: Int, l: Int = 2): Seq[Boolean] = {
    for{
      i <- - l until size + l
    } yield beam(x + i, y + size - 1 - i)
  }

  def showNear(x: Int, y: Int, l: Int = 2): Unit = {
    println(s"Near ($x,$y)")
    val d = Display.of[Char]((x-l,y-l),(2*l + 1,2*l + 1))(visibleBeam)
    println(d.showDisplay()())
  }
  ...
  lazy val answer2: Int = {
    ...
    val res = getDiagonalBottomLeftTopRightWiderByL(x,y) //650 360 = 59; 850 400 78 1090,662 99; 1099,667 = 100; 1097,666=100 1107, 673 = 101; 1099,668 = 100; 1097,667
    println(show(res))
