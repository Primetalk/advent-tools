...
import org.primetalk.advent.tools.PrimeNumbers.longPow
...
object Day2014 extends Utils {
  ...
  def parseProgram(p: String): List[Op] = {
    ...
    def maskOp[_: P]: P[Op] = P(
      "mask = " ~ CharIn("10X").rep.!.map(s => parseBitmask(s.toCharArray.toList))
    )

    def memOp[_: P]: P[Op] = P(
      ("mem[" ~  positiveLong ~ "] = " ~ positiveLong)
        .map(mem.tupled)
    )
    ...
    def program[_: P]: P[Seq[Op]] = P(
      op.rep
    )

    parse(p, program(_)).get.value.toList
  }
...
//    )
  ...
  val all36Bits: Long = longPow(2, 36) - 1
