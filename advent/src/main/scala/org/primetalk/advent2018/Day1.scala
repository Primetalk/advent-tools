...
object Day1 extends Utils {
  ...
  def seqOfChanges: Seq[Change] =
    inputText.map(_.toInt)

  /** The frequency is just the sum of all changes. */
  def frequency(changeList: Seq[Change]): Int =
    changeList.sum

  /**
    *
    * @param input  New-line separated list.
    * @return Frequency
    */
  ...
  def duplicateFrequencySearchViaStream(currentFrequency: Int, visitedFrequencies:  mutable.TreeSet[Int], changes: Stream[Change]): Int = {
    if(visitedFrequencies.contains(currentFrequency))
      ...
      duplicateFrequencySearchViaStream(changes.head, visitedFrequencies, changes.tail)
    }
  }
  ...
  def duplicateFrequencySearch(currentFrequency: Int, visitedFrequencies:  mutable.TreeSet[Int], changes: Seq[Change], allChanges: Seq[Change]): Int = {
    if(visitedFrequencies.contains(currentFrequency))
      ...
      if(changes.isEmpty)
          duplicateFrequencySearch(currentFrequency, visitedFrequencies, allChanges, allChanges)
      else {
          visitedFrequencies += currentFrequency
          duplicateFrequencySearch(currentFrequency + changes.head, visitedFrequencies, changes.tail, allChanges)
      }
    }
  }

  def findFirstDuplicateFrequency: Int =
