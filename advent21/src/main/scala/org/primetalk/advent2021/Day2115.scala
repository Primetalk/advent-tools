...
/**
  ...
  * 
  * Your puzzle answer was 2831.
  * 
  * Both parts of this puzzle are complete! They provide two gold stars: **
  */
object Day2115 extends Utils:

  val input = readThisObjectInputLines

  val maze: IDisplay2D[Char] = 
    IDisplay2D.readCharDisplay(input)

  case class PathInfo[T](totalRisk: Int, reversePath: List[T]):
    def node: T = reversePath.head
    def prepend(h: T, risk: Int): PathInfo[T] = 
      PathInfo(risk + totalRisk, h :: reversePath)
    ...
    val found = GraphUtils.priorityFindFirst[PathInfo[Position], PathInfo[Position], Map[Position, PathInfo[Position]]](
      map => path => {
        ...
        if map.contains(key) && map(key).totalRisk < path.totalRisk then
          ...
          val nextPoints = maze.adjacentPositions(key)
  ...
  //Part 2
  val maze2: IDisplay2D[Char] = {
    val mazeH = IDisplay2D.fromFunction(Rectangle((0,0), (5,5))){p =>
      p._1 + p._2
    }
    given Conversion[Int, IDisplay2D[Char]] with
      def apply(i: Int): IDisplay2D[Char] = maze.map{ ch =>
        val res: Int = ch - '0' + i 
        (
          (
            if res > 9 then
    ...
    val mazeD: IDisplay2D[IDisplay2D[Char]] = 
      mazeH.map(summon[Conversion[Int, IDisplay2D[Char]]])

    given Conversion[IDisplay2D[Char], IDisplay2D[Char]] = identity
    mazeD.flatten[Char]
  }
  // 2590 - low
