...
/**
  ...
  */
object Day2125 extends Utils:

  val input = readThisObjectInputLines

  val initialDisplay = IDisplay2D.readCharDisplay(input)

  def moveRight(d: IDisplay2D[Char]) = d.produceByGlobalRules{
    case (p, '.') =>
      val leftPosition = (if p._1 == d.minX then d.maxX else p._1 - 1, p._2)  
      if d(leftPosition) == '>' then
        '>'
      else
        '.'
    case (p, '>') =>
      val rightPosition = (if p._1 == d.maxX then d.minX else p._1 + 1, p._2)  
      if d(rightPosition) == '.' then
        '.'
      else
        '>'
    case (_, v) => v
  ...
  def moveDown(d: IDisplay2D[Char]) = d.produceByGlobalRules{
    case (p, '.') =>
      val upPosition = (p._1, if p._2 == d.minY then d.maxY else p._2 - 1)  
      if d(upPosition) == 'v' then
        'v'
      else
        '.'
    case (p, 'v') =>
      val downPosition = (p._1, if p._2 == d.maxY then d.minY else p._2 + 1)  
      if d(downPosition) == '.' then
        '.'
      else
        'v'
    case (_, v) => v
  ...
  val step = moveRight andThen moveDown

  lazy val answer1: Long =
    NumberSequenceUtils.countUntilFixedPoint(initialDisplay)(step, _.isEqualTo(_))._2 
      + 1
