...
Your puzzle answer was 20=02=120-=-2110-0=1.
--- Part Two ---
...
Both parts of this puzzle are complete! They provide two gold stars: **
  */
object Day2225 extends Utils:

  def parseSNAFU(snafu: String): Long =
    def parseSNAFU0(chars: List[Char], accum: Long = 0L): Long =
      chars match
        case head :: tail => 
          val current = head match
            ...
            case '-' => -1
            case _ => head - '0'
          parseSNAFU0(tail, accum * 5 + current)
        case Nil =>
          accum
    parseSNAFU0(snafu.toCharArray().toList)

  def toSNAFU(num: Long): String = 
    def toSNAFU0(num: Long, result: List[Char] = Nil): String =
      if num == 0L then
        ...
        val ch = m match
          ...
          case 4 => '-'
          case _ => (m + '0').toChar
        val next = num / 5 + (if m >=3 then 1 else 0)
        toSNAFU0(next, ch :: result)
    toSNAFU0(num)

  val numbers = readThisObjectInputLines.map(parseSNAFU)
  // 1-=01=11-=2-210-=-=1
  lazy val answer1: String = 
    println(parseSNAFU("1=-0-2"))
    println(toSNAFU(4890))
    toSNAFU(numbers.sum)
