...
object Day2219 extends Utils:
  ...
  // Important! should be large enough for considering potential variations.
  val maxStepsPerTime = 400

  def generateAll(maxTime: Int)(initialState: State): Int =
    GraphUtils.enumerateSearchSpace[State, (Int, Int)](
      next = step1(maxTime),
      aggregate = (p, s) => 
        val (epoch, sc) = p
       // if epoch % maxStepsPerTime == 0 then println(epoch)
        (epoch + 1, math.max(sc, score(s))),
      eliminate = (p, s) => 
        val (epoch, sc) = p
