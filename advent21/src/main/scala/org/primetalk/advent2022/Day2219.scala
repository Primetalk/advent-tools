...
Your puzzle answer was 21840.

Both parts of this puzzle are complete! They provide two gold stars: **
  */
object Day2219 extends Utils:
  ...
  case object Ore extends ResourceKind
  case object Clay extends ResourceKind
  case object Obsidian extends ResourceKind
  case object Geode extends ResourceKind

  sealed trait Item:
    def kind: ResourceKind
  ...
  case class Resource(kind: ResourceKind) extends Item
  case class Robot(kind: ResourceKind) extends Item
  case class RobotBlueprint(robotKind: ResourceKind, items: List[(Resource, Int)])

  type BuildingPlan = List[Option[Robot]]
  type BuildingPlan2 = List[(Int, Robot)]

  case class Blueprint(id: Int, robots: List[RobotBlueprint])

  val resourceKind: Parser[ResourceKind] =
    Parser.string("ore").as(Ore) |
    Parser.string("clay").as(Clay) |
    Parser.string("obsidian").as(Obsidian) |
    Parser.string("geode").as(Geode)

  val resourceAmount: Parser[(Int, Resource)] =
    ParsingUtils.positiveNumber ~ (Parser.char(' ') *> resourceKind).map(Resource.apply)

  // Each obsidian robot costs 2 ore and 20 clay.
  val robotBlueprint: Parser[RobotBlueprint] = 
    ((Parser.string("Each ") *> resourceKind <* Parser.string(" robot costs ")) ~ resourceAmount.repSep(Parser.string(" and ")) <* Parser.string("."))
      .map((rk, rs) => RobotBlueprint(rk, rs.map(_.swap).toList))

  given blueprint: Parser[Blueprint] =
    ((Parser.string("Blueprint ") *> ParsingUtils.positiveNumber <* Parser.string(": ")) ~
      robotBlueprint.repSep(Parser.char(' ')).map(_.toList))
      .map((id, robots) => Blueprint(id, robots.reverse))// we'll build robots in a priority order
  ...
  val blueprints = input.split("\n").map(_.parseUnsafe[Blueprint])

  type Resources = Map[ResourceKind, Int]
  type Robots = Map[ResourceKind, Int]

  def addMap[K](m1: Map[K, Int], m2: Map[K, Int]): Map[K, Int] =
    m2.foldLeft(m1){ case (m, (k,cnt)) => m.updated(k, m.getOrElse(k, 0) + cnt)}

  def inc[T](m: Map[T, Int], k: T): Map[T, Int] = 
    m.updated(k, m.getOrElse(k, 0) + 1)

  val initialRobots: Robots = Map(Ore -> 1)
 
  // This is the key to fast progress.
  // We value each subsequent kind of resource/robot 10x more than the previous level.
  val prices = Map(
    Resource(Ore) -> 1,
    Robot(Ore) -> 10,
    Resource(Clay) -> 100,
    Robot(Clay) -> 1000,
    Resource(Obsidian) -> 10000,
    Robot(Obsidian) -> 100_000,
    Resource(Geode) -> 1_000_000,
    Robot(Geode) -> 10_000_000,
  )

  case class State(blueprint: Blueprint, resources: Resources, robots: Robots, time: Int):
    val total: Int =
      resources.map((item, cnt) => prices.getOrElse(Resource(item), 1) * cnt).sum +
        robots.map((item, cnt) => prices.getOrElse(Robot(item), 1) * cnt).sum

  def harvest(robots: Robots, resources: Resources): Resources =
    addMap(resources, robots)

  def consume(resources: Resources, robotBlueprint: RobotBlueprint): Resources = 
     robotBlueprint.items.foldLeft(resources){ 
        case (resources, (res, req)) => 
          val remaining = resources.getOrElse(res.kind, 0) - req
          require(remaining >= 0, s"resource = $res, req = $req, resources: $resources,")
          resources.updated(res.kind, remaining)
      }

  @tailrec
  def buildRobots(robots: List[RobotBlueprint], resources: Resources, result: List[(ResourceKind, Resources)] = Nil): List[(ResourceKind, Resources)] =
    robots match
      case head :: tail => 
        val building = head.items.forall((r, req) => 
              resources.getOrElse(r.kind, 0) >= req)
        val result2 =
          if building then 
            (head.robotKind, consume(resources, head))::result
          else
            result
        // NB! this heuristic significantly reduces search space
        if building && (head.robotKind == Geode || head.robotKind == Obsidian) then 
          result2
        else
          buildRobots(tail, resources, result2)
      case Nil =>
        result

  def step1(maxTime: Int)(s: State): List[State] =
    if s.time == maxTime then 
      Nil 
    else
      val newRobots = buildRobots(s.blueprint.robots, s.resources)
      val stateWithoutBuildingAnyRobot = State(s.blueprint, harvest(s.robots, s.resources), s.robots, s.time + 1)
      val nextStates = stateWithoutBuildingAnyRobot ::  newRobots.map{
        case (newRobotKind, resourcesAfterStartBuildingOfStep) =>
          val inventoryAfterHarvest = harvest(s.robots, resourcesAfterStartBuildingOfStep)
          State(s.blueprint, inventoryAfterHarvest, inc(s.robots, newRobotKind), s.time + 1)
      }

      nextStates

  given stateOrder: Order[State] = Order.reverse( Order.from{ (s1, s2) => 
    val result = s2.time - s1.time // width-first over time
    if result == 0 then
      s1.total - s2.total
    else
      result
  ...
  def score(s: State): Int =
    s.resources.getOrElse(Geode, 0)

  // Important! maxStepsPerTime should be large enough for considering potential variations.
  def generateAll(maxTime: Int, maxStepsPerTime: Int = 19)(initialState: State): Int =
    GraphUtils.enumerateSearchSpace[State, (Int, Int)](
      next = step1(maxTime),
      aggregate = {
        case ((epoch, geodeCount), s) => 
          (epoch + 1, math.max(geodeCount, score(s)))
      },
      eliminate = {
        case ((epoch, _), s) => 
  ...
  lazy val answer1: Int =
    val maxTime = 24
    blueprints.par.map{ blueprint => 
      val res = generateAll(maxTime)(State(blueprint, Map(), initialRobots, 0))
      res * blueprint.id
    }.sum
  //Part 2 238 low 21840
  lazy val answer2: Int =
    val maxTime = 32
    blueprints.take(3).par.map{ blueprint => 
      val res = generateAll(maxTime, 322)(State(blueprint, Map(), initialRobots, 0))
      res
    }.product
